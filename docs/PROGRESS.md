...
**Degisiklikler:**
- **Yeni model:** `emptyStateModel.ts` — `deriveEmptyStateModel()` pure fonksiyonu ve `getGreeting()`, `getProjectNameFromWorkingDirectory()` helper'lari. Model `contextLine`, `contextChips`, `suggestions` (4 adet: code, review, research, document) ve `lead` (`Nereden baslayalim?`) doner. Edge-case'ler: drive root (D:\ -> null), empty/whitespace device label -> null.
...
- **CSS:** Sadece `components.css` icinde yeni class'lar: `.runa-chat-empty-hero__context`, `.runa-chat-empty-context`/`__chip`, `.runa-chat-suggestion__copy`/`__label`/`__description`. `primitives.css` kapsam disi tutuldu.
- **Testler:**
  - `emptyStateModel.test.ts`: 31 test — model unit + edge-case (root path, empty device).
  - `EmptyState.test.tsx`: 27 test — component render, context, suggestions, `container.textContent` ile forbidden/mojibake taramasi, tam path leak, empty/negative edge-case.
  - `ChatFirstShell.test.tsx`, `CopyVoicePass.test.tsx`: assertion guncellendi.
  - `design-language-lock.test.ts`: empty state BOM, mojibake, forbidden technical strings, CSS contract guard'lari genisletildi; fixture dosyalari da BOM guard kapsaminda.
- **Visual smoke:** Gercek React `EmptyState` component'ini render eden TSX fixture + Playwright spec. Dogrulamalar: greeting, lead, project chip (tam path leak yok), device chip, conversation chip, 4 suggestion label, click callback ile `selected-prompt` dolmasi, forbidden technical strings, mojibake, horizontal overflow (390/320px).
