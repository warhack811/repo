...
- `docs/UI-OVERHAUL-07-APPROVAL-UX-STATE-FEEDBACK-PROMPT.md` eklendi. Prompt, `Approval UX & State Feedback` kapsamÄ±nÄ±n `docs/UI-OVERHAUL-07.md` icinde gercekte 7.3 oldugunu ve 7.4'un `Operator/Developer Hard Isolation` oldugunu acikca kaydediyor; approval kapsaminda kalinmasi ve hedef/path uydurulmamasi guardrail olarak yazildi.
...
- Production build kaniti: onceki global CSS follow-up hedefinde `dist/assets/index-*.css` yaklasik `143.66 kB` idi; bu turda global kritik CSS chunk'i `50.13 kB` raw / `9.04 kB` gzip seviyesine indi. Initial stylesheet toplamÄ± `index + RunaBadge + RunaSurface/types` olarak yaklasik `51.60 kB` raw; chat route CSS'i ayri `63.99 kB` raw / `6.55 kB` gzip lazy chunk oldu.
...
- Env fallback seami `DEEPSEEK_API_KEY` ile acildi; `approval-store` secret minimization DeepSeek icin de server env varsa persisted request-only key'i bosaltir. `.env-Ã¶rnek` ve `.env.server.example` icine `DEEPSEEK_API_KEY`, `DEEPSEEK_FAST_MODEL`, `DEEPSEEK_REASONING_MODEL`, `RUNA_DEEPSEEK_MODEL_ROUTER_ENABLED` ve `RUNA_MODEL_ROUTER_ENABLED` placeholder'lari eklendi.
...
- Dogrulama:
  ...
  - `pnpm.cmd exec biome check packages/types/src/ws.ts apps/server/src/gateway/config-resolver.ts apps/server/src/persistence/approval-store.ts apps/server/src/gateway/fallback-chain.ts apps/server/src/gateway/model-router.ts apps/server/src/gateway/factory.ts apps/server/src/gateway/deepseek-gateway.ts apps/server/src/gateway/gateway.test.ts apps/server/src/gateway/model-router.test.ts apps/server/package.json apps/server/scripts/deepseek-live-smoke.mjs .env.server.example .env-Ã¶rnek` PASS
...
- `.env.server.example` ve `.env-Ã¶rnek` icine local vision smoke alanlari eklendi: `OPENAI_API_KEY`, `RUNA_OPENAI_BASE_URL`, `LMSTUDIO_BASE_URL`, `LMSTUDIO_MODEL`, `LMSTUDIO_API_KEY`, `RUNA_OPENAI_COMPAT_ALLOW_REMOTE`.
...
### Track C / TASK-01 Final Closure - Packaged Electron Main Process-Liveness KanÄ±tÄ± - 25 Nisan 2026

- `TASK-01-ELECTRON-DESKTOP-APP.md` kapsamÄ±ndaki en kritik blocker olan "packaged app sessizce exit code 0 ile kapanÄ±yor" sorunu Ã§Ã¶zÃ¼ldÃ¼. Bu Ã§Ã¶zÃ¼m 3 farklÄ± root-cause alanÄ±nÄ± adresledi.
- **Top-Level Await Fix:** Packaged Electron main process V8 context'i ESM top-level await'i desteklemediÄŸi iÃ§in uygulama sessizce kapanÄ±yordu. `apps/desktop-agent/electron/main.ts` entry point'i `app.whenReady().then(...)` zincirine Ã§evrildi ve boot-liveness saÄŸlandÄ±.
- **Renderer Bundle & Asar Path Fix:** Renderer HTML'in asar iÃ§inde dÄ±ÅŸarÄ± Ã§Ä±kmaya Ã§alÄ±ÅŸan `../../dist-electron/` yollarÄ± asar file-system abstraction'Ä±nÄ± kÄ±rdÄ±ÄŸÄ± iÃ§in renderer asset'leri doÄŸrudan `dist-electron/electron/renderer/` altÄ±na kopyalanacak ÅŸekilde `package.json` build script'leri gÃ¼ncellendi.
- **Ä°kili Build Output SadeleÅŸtirmesi:** `electron-builder.yml` dosyasÄ±nda `dist/` ve kaynak dosyalar Ã§Ä±karÄ±ldÄ±, yalnizca `dist-electron/` klasÃ¶rÃ¼ paketlendi. Bu sayede module duplicate riski onarÄ±ldÄ± ve `app.asar` boyutu optimize edildi.
- DoÄŸrulama:
  - `pnpm.cmd --filter @runa/desktop-agent typecheck` PASS
  - `pnpm.cmd --filter @runa/desktop-agent build` PASS
  - `pnpm.cmd --filter @runa/desktop-agent dist:win` PASS
  - Packaged Execution Smoke: `release\win-unpacked\Runa Desktop.exe` Ã§alÄ±ÅŸtÄ±rÄ±ldÄ± ve boot loglarÄ± kontrol edildi (`boot:module-loaded`, `boot:app-ready`, `window:did-finish-load` zinciri eksiksiz gÃ¶rÃ¼ldÃ¼).
- Durum: `TASK-01` artÄ±k release-grade process-liveness, UI render ve installer gereksinimlerini saÄŸlamÄ±ÅŸ durumdadÄ±r.
...
- Quit davranisi tray/uygulama kapanisinda ayri ele alindi: main-process `before-quit` uzerinden `controller.stop()` calistiriyor, tray'i destroy ediyor ve session'i silmeden runtime'i kapatarak uygulamayi kapatiyor. `Sign out` ise mevcut 01B persistence semantiÄŸiyle session'i temizliyor. Auto-start icin `app.setLoginItemSettings()` benzeri bir wiring bilincli olarak eklenmedi; tray menude yalniz `Launch at sign-in: Off` gorunur durumda ve varsayilan kapalilik korunuyor.
...
- Durust kalan durum: Bu tur tray icon'un gorsel varligi ve menu etkileÅŸimi manuel olarak gorulmedi; smoke yalniz runtime ayakta kalma seviyesinde kanit verdi. Native OS autostart, updater, installer ve signed release packaging halen `TASK-01D` veya sonraki hardening konusu.
...
- Kullanici key girebilsin diye `.env` icine `SAMBANOVA_API_KEY=` alani, takip edilebilir sablonlar icin de `.env-Ã¶rnek` ve `.env.server.example` alanlari eklendi.
...
- Dogrulama:
  - `pnpm.cmd exec biome check packages/types/src/ws.ts apps/server/src/gateway/config-resolver.ts apps/server/src/persistence/approval-store.ts apps/server/src/gateway/fallback-chain.ts apps/server/src/gateway/model-router.ts apps/server/src/gateway/factory.ts apps/server/src/gateway/sambanova-gateway.ts apps/server/src/gateway/gateway.test.ts .env-Ã¶rnek .env.server.example` PASS
  - `pnpm.cmd --filter @runa/types typecheck` PASS
  - `pnpm.cmd --filter @runa/server typecheck` PASS
  - `pnpm.cmd --filter @runa/web typecheck` PASS
...
- `apps/web/src/components/chat/PastRunSurfaces.tsx` eklendi. `pastPresentationSurfaces.map(...)` bloÄŸu typed prop'larla ayrildi; expanded state, inspection detail action state, pending keys, transport summaries ve approval resolve callback'leri aynen korunuyor.
...
- `apps/server/src/ws/run-execution.ts` artik authenticated run baslangicinda `monthly_turns` metriÄŸi icin `ws_run_request` limitini enforce ediyor. Limit asiminda `run.accepted` gonderilmeden once kontrollu `run.rejected` donuyor; ayni kullanici icin minute-window dolunca typed neden payload'a dusuyor.
...
- Root seviyesinde `@playwright/test` devDependency'si, `test:e2e` script'i, yeni `playwright.config.ts`, `.github/workflows/ci.yml` ve `e2e/*` smoke altyapisi eklendi. AmaÃ§ canli provider secret'i kullanmadan auth bootstrap + chat submit + approval replay yolunu release oncesi minimum kalite kapisi olarak kanitlamakti.
- `e2e/serve-runa-e2e.mjs` deterministic bir local harness sagliyor: Fastify + WS server dist output'u ayaga kaldiriliyor, local-dev auth bootstrap aktif ediliyor, OpenAI chat-completions cagrisi process-ici mock ile intercept ediliyor ve approval sonrasi `file.write` replay'i gercek tool registry uzerinden proof dosyasi yazarak tamamlanÄ±yor.
...
- Kalan blocker dar kapsamda yeniden incelendi. Kod taramasi gosterdi ki request assembly zinciri `file.list` schema'sini dogru tasiyor; asimetrik kirilma, ayni schema ile bazen `include_hidden: false`, bazen `include_hidden: "false"` ya da `False` benzeri provider-side generation varyansi olarak ortaya cikiyordu. Bu nedenle sorun yalnÄ±z `request-tools` serializer bug'i diye siniflandirilmadi.
...
- Durust sonuc: bu turda kapanan alan exact compatibility closure'dur. Kalan residual risk, Groq tarafinda genel tool-call generation variansi ihtimalinin tamamen teorik olarak yok oldugu degil; ama bugunku hedef matrix ve mevcut runtime yollar icin blocker kapanmis, canlÄ± kanit PASS ile alinmistir.
...
- `apps/server/scripts/approval-browser-authority-check.mjs` eklendi ve `apps/server/package.json` icine `test:approval-browser-authority-check` komutu baglandi; harness built server + Vite dev + headless Edge CDP uzerinden gercek tarayici akisini koÅŸturuyor.
...
- Teknik izolasyonun URL bypass ile delinmemesi icin `apps/web/src/pages/DashboardPage.tsx` dar kapsamda gate'lendi; `OperatorControlsPanel` ve `TransportMessagesPanel` de yalniz Developer Mode acikken gorunur kaldÄ±.
...
- `apps/web/src/pages/ChatPage.tsx` buyuk olcude sadeleÅŸtirildi ve 462 satira indirildi; presentation orchestration ve run-surface rendering yukunun ana kismi yeni `apps/web/src/components/chat/chat-presentation.tsx` ve `apps/web/src/components/chat/PresentationRunSurfaceCard.tsx` dosyalarina tasindi.
...
- `apps/web/src/components/chat/PresentationBlockRenderer.tsx` ile `apps/web/src/lib/chat-styles.ts` uzerinde code/diff/web-search/tool-result yuzeyleri daha okunakli premium kartlara cekildi; paylasilan stil objeleri CSS variable tabanli renkler ve yumusak transition'larla sadeleÅŸtirildi.
...
- Durust durum: approval E2E bu turda gecmis sayilmadi. Browser/storage tarafinda kalici bir Groq key bulunamadigi ve current shell env de bos oldugu icin gerÃ§ek provider repro ve `Kabul Et / Reddet -> continuation -> terminal result` zinciri yeniden kosturulamadi.
...
- Sonraki onerilen gorev: chat giris panelindeki operator dili/copy'sini urun diliyle sadeleÅŸtirip teknik panelleri varsayilan yuzeyden daha da geri cekmek.
...
- Sonraki onerilen gorev: chat page icin product-first copy sadeleÅŸtirmesi ve operator/debug yuzeylerini varsayilan deneyimden daha da geriye cekmek.
...
- `apps/server/src/runtime/run-model-turn.ts`, `apps/server/src/runtime/run-model-turn-loop-adapter.ts`, `apps/server/src/runtime/run-agent-loop.ts` ve `apps/server/src/runtime/agent-loop.ts` tarafinda coklu `tool_results` snapshot boyunca tasinabilir hale getirildi. Consecutive tool failure sayaci tek tek result yerine batch semantigine gore davranÄ±yor: batch icinde en az bir basari varsa sayac sifirlaniyor, tum batch hata ise tek tur failure olarak artis oluyor.
...
- Duru not: `src/ws/register-ws.test.ts` dosyasinin tumune kosulan onceki genis testte `web.search` authority metin beklentisinden gelen task-disi, onceki baseline drift goruldu. Task 04 kapaniÃ…Å¸ kaniti bu nedenle task'a ait integration senaryolarini hedefleyen pattern bazli Vitest kosusuna dayaniyor; paralel tool scheduling/runtime seam'i icin gerekli proof mevcuttur.
...
- Serper answer box ve knowledge graph alanlari structured output olarak tasiniyor; snippet/title/source alanlari prompt-control tag sanitization katmanindan geciyor. Organic default davranis ve freshness bias (`tbs=qdr:m`) korunuyor; news modunda freshness note provider date/snippet gecikmesi konusunda uyarÄ±yor.
- Trust-tier siniflandirmasi official/reputable kaynaklar icin genisletildi: resmi kurum/edu/gov, docs-like host/path, arxiv/wikipedia/akademik ve reputable tech kaynaklari ayrisiyor. `researchgate.net`, `scholar.google.com` ve community/forum host segmentleri otomatik high-trust yapilmiyor; docs-like path tasisa bile lower-trust kalÄ±yor.
- Dogrulama yesil:
...
- `docs/UI-OVERHAUL-01.md`, `docs/UI-OVERHAUL-02.md` ve `docs/UI-OVERHAUL-03.md` kapsamÄ± repo Ã¼zerinde kapatÄ±ldÄ±: DeveloperPage ayrÄ±mÄ± korunurken chat yÃ¼zeyinde developer/raw/operator dilini yakalayan manifesto gate yeÅŸil kaldÄ±; CSS entry `apps/web/src/styles/index.css` Ã¼zerinden token/reset/primitives/animations/component katmanÄ±na baÄŸlandÄ±; `apps/web/src/index.css` tek import entry haline getirildi.
- Inline style migration tamamlandÄ±: `apps/web/src/components/**` ve `apps/web/src/pages/**` altÄ±nda case-sensitive `style=`, `CSSProperties`, `chat-styles` taramasÄ± `0`; `apps/web/src/lib/chat-styles.ts` silindi. Legacy inline stiller token-backed CSS Ã§Ä±ktÄ±sÄ±na taÅŸÄ±ndÄ± ve `scripts/ci/style-check.mjs` ile tekrar kaÃ§masÄ± engellendi.
- Runa primitive seti geniÅŸledi: mevcut `RunaButton`, `RunaCard`, `RunaSurface`, `RunaTextarea`, `RunaBadge` CSS Module tabanÄ±na taÅŸÄ±ndÄ±; `RunaInput`, `RunaModal`, `RunaSheet`, `RunaToast`, `RunaSkeleton`, `RunaSpinner`, `RunaIcon`, `RunaTooltip`, `RunaDisclosure` eklendi.
- Yeni CI yÃ¼zeyleri: `pnpm run style:check` fail-fast inline-style gate'i; `pnpm run primitive:coverage` non-blocking primitive/native kullanÄ±m raporu. `biome.json` iÃ§inde Vite CSS module index-signature davranÄ±ÅŸÄ±yla Ã§akÄ±ÅŸan `useLiteralKeys` kuralÄ± kapatÄ±ldÄ±.
- Ä°lk karÅŸÄ±lama copy testi dÃ¼zeltildi: `EmptyState` artÄ±k `Bugun neyi birlikte ilerletelim?` ve `onay isteyen` Ã¶neri dilini render ediyor.
- DoÄŸrulama yeÅŸil:
  - `pnpm.cmd --filter @runa/web typecheck`
  - `pnpm.cmd --filter @runa/web lint`
  ...
  - `pnpm.cmd run primitive:coverage` (`REPORT`; Runa primitive total `27`, native interactive total `39`, fail deÄŸil takip metriÄŸi)
- Browser QA: Playwright ile `http://localhost:5173/chat` local-dev authenticated akÄ±ÅŸÄ±nda `320x700`, `768x900`, `1440x1000` viewport smoke PASS. `hashCleared=true`, chat header ve composer gÃ¶rÃ¼nÃ¼r, ana chat yÃ¼zeyinde `Raw Transport` / `Model Override` yok, console error/pageerror `0`. Screenshot kanÄ±tlarÄ± local ve gitignore altÄ±nda: `apps/web/.qa-screenshots-auth-chat/`.
- Kalan not: `primitive:coverage` native interactive kullanÄ±mÄ±nÄ±n hÃ¢lÃ¢ primitive kullanÄ±mÄ±ndan yÃ¼ksek olduÄŸunu raporluyor; bu UI-OVERHAUL-03 done kriterinde fail deÄŸil, UI-04/05 split-polish sÄ±rasÄ±nda doÄŸal migration metriÄŸi olarak izlenmeli.
...
- Typography tokenlari Inter uzerinden sadeleÅŸtirildi: `--font-sans`, heading/body scale tokenlari, line-height tokenlari ve `--tracking-heading: 0` eklendi. Viewport-width font scaling kullanan aktif style noktalar temizlendi.
- Brand asset seami eklendi: `apps/web/src/assets/runa-logo.svg` ve public `favicon.svg` Runa markasina hizalandi.
- LoginPage tek odakli marka/form yuzeyine indirildi. Dev/token yollarÄ± `Diger giris yontemleri` disclosure altinda kalir; email/password ve OAuth akisi onde. Line-count: `LoginPage.tsx` 212 satir.
...
- Kapsam: `docs/UI-OVERHAUL-07-5-SECONDARY-SURFACES-REFRAME-PROMPT.md` promptuna bagli kalinarak History, Devices ve Account/Settings yuzeyleri consumer-grade secondary surface olarak sadelestirildi. Server/auth/desktop-agent/protocol kontratlari degistirilmedi; yeni dependency eklenmedi.
...
- Kapsam: `docs/UI-OVERHAUL-07.md` icindeki 7.6 gercegine bagli kalinarak web UI renk, type, border/radius ve shadow dili tek hatta indirildi. Server, desktop-agent, auth, websocket, provider ve persistence kontratlari degistirilmedi; yeni dependency eklenmedi.
...
- Kapsam: `docs/UI-OVERHAUL-07.md` icindeki 7.8 hedeflerine bagli kalinarak normal kullanici yuzeylerinde command palette, keyboard/focus flow, mobile composer dayanÄ±kliligi, microinteraction ve sakin loading state polish'i uygulandi. Server, desktop-agent, auth, websocket, provider ve persistence kontratlari degistirilmedi; yeni dependency eklenmedi.
...
- Kapsam: `docs/UI-OVERHAUL-07-9-FINAL-COHERENCE-AUDIT-DESIGN-LANGUAGE-LOCK-PROMPT.md` promptuna bagli kalinarak UI-OVERHAUL-07 final coherence audit'i uygulandi. Server, desktop-agent, auth, websocket, provider ve persistence kontratlari degistirilmedi; yeni dependency eklenmedi.
...
- `apps/server/src/persistence/approval-store.ts` icinde continuation persistence hattÄ± daraltildi. `provider_config` artik spread edilerek kalici yazilmiyor; persistence icin explicit minimum shape kuruluyor: yalniz `apiKey` ve ancak continuation request'i onlar olmadan calisamayacaksa `defaultModel` / `defaultMaxOutputTokens` tutuluyor.
...
- Prompt'lar artik "genel feature istegi" degil, repo icinde dar kapsamli ve additive koÅŸturulabilir gorev formatinda. Her konuda sebep-sonuc kisa notu, degistirilebilecek dosyalar, degistirilmeyecek dosyalar ve test/validation kapilari acikca yaziyor.
